...
class StoreDataApi(Resource):
    def post(self):
        ...
        if exists is not None:
            token = create_access_token(identity={'username': username, "data": [
                                        order.convertToDict() for order in orders]})
    ...
    def delete(self):
        ...
        if res1 is not None:
            res2 = next(filter(lambda x: x.itemname ==
                        itemname, res1.items), None)
            if res2 is not None:
